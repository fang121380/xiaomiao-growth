...
- 📄 **PDF 导出**：Canvas 渲染 A4 成长摘要卡，含小猫资料 + 统计数字 + 类型分布 + 最近记录
- 🎉 **里程碑 / 月报分享卡**：一键生成可分享图片（Android 原生分享面板，兜底下载）
- 🌙 **深色模式**：3 选项（浅色 / 深色 / 跟系统），所有 CSS 变量自动重写，毛玻璃质感保留
- 🛡️ **AI 容错**：20s 超时 + 2 次重试 + 友好错误提示（区分超时 / API / 网络错）
...
`xiaomiao-v2.3.0.apk` —— 远程拉 `https://xiaomiao-toh.pages.dev/`

**v2.3.0 新增**：
- 🌙 **深色模式**（设置页 3 选项：浅色 / 深色 / 跟系统，毛玻璃质感保留）
- 📄 **PDF 导出**（Canvas 渲染 + A4 成长摘要卡，中文完美，可分享家人）
- 🎉 **里程碑 / 月报分享卡**（一键生成可分享图片，Android 原生分享面板）
- 🛡️ **AI 容错**（20s 超时 + 2 次重试 + 友好错误提示，再也不会死转圈）
